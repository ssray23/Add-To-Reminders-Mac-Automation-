...
- **Details**: Instead of relying on a cloud-based LLM or API, the parser uses a combination of custom Regular Expressions and Apple's incredibly fast `NSDataDetector`. 
  ...
  - **Relative Parsing**: Evaluates phrases like "in 3 hours", "in 6 months from now", or "in 2 years" manually via regex, strictly requiring "in" or "from now" keywords to prevent false positives.
  - **Absolute Parsing & Multiple Dates**: Passes the scrubbed text to `NSDataDetector` to find dates and URLs. It features a robust two-pass merging strategy to handle scenarios where the date and time are provided separately (e.g. on different lines). It accumulates all detected dates (both relative and absolute) and subsequently removes their string matches from the final title. It overrides the default `12:00 PM` to `7:00 AM` if no explicit time was given. It also implements a clever zero-width space injection trick (`\u{200B}`) to prevent `NSDataDetector` bugs with words like "due".
