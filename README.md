...
## Features
- **Relative & Absolute Date Parsing:** Supports relative times explicitly marked with keywords like "in 3 hours", "30 mins from now", "in 2 weeks", "over the weekend", "this weekend", "next weekend", "end of week", or "end of month" via custom phrase resolvers, and seamlessly uses Apple's on-device `NSDataDetector` for absolute dates.
- **Multiple Date Selection:** If multiple potential dates or weekend options (e.g. Saturday vs Sunday) are detected in your highlighted text, the Quick Entry window dynamically adapts to display an elegant radio-button list of the dates, letting you pick the right one seamlessly alongside your title review.
...
- **Recurrence Rules:** Natural language parsing for repeating events! Supports specific day-of-week and multi-day patterns (e.g. "every Friday", "every Fri", "every other Monday", "every Mon, Wed, Fri", "every Mon to Fri", "repeat on Fridays", "on Mondays and Thursdays"), frequency intervals (e.g. "every 3 days", "every 2 weeks", "every 1 month", "every other week"), explicit unit patterns ("repeat year", "repeat month", "repeat week", "repeat day", "annually", "repeat daily", "weekly", "yearly"), and duration-based recurrences like "for next 4 days" or "daily for 7 days" (which automatically start today at default 7:00 AM and repeat through the specified duration). You can even specify an end date by appending "until [date]" or "ending on [date]".
- **Smart Fallbacks:** Handles an extensive list of common typos, shorthand (e.g., "2nite", "tmrw", "yea", "yealy", "anually"), and "fat-finger" mistakes (e.g., "minuets", "fortnite") specifically for the date/time engine while intentionally preserving your exact phrasing and brand names in the actual reminder title. It also automatically normalizes erratic whitespace (tabs, newlines) when parsing, so copying tabbed table rows works perfectly. It parses spelled-out ordinals (e.g. "Fourth of July" -> "4th of July"), and gracefully defaults to 7:00 AM for dates without specific times. It perfectly mitigates `NSDataDetector` bugs that misinterpret words like "due", "before", "by", or "until" as durations starting from today.
- **Clean Titles:** Automatically strips out empty parentheses, dangling prepositions (e.g. "Expires"), and both leading and trailing punctuation (like leading dots or dashes) left behind after date extraction to ensure pristine reminder titles.
...
- **`tests/RegressionTests.swift`:** Automated test suite running 181 assertions on parser logic, day-of-week recurrences, date range preservation, frequency and interval formatting, unit recurrence patterns ("repeat year", "annually"), relative date calculation, typo fixes ("yea"), tricky words, URL extraction, and recurrence rules.
