...
- **Recurrence Rules:** Natural language parsing for repeating events! Supports specific day-of-week and multi-day patterns (e.g. "every Friday", "every Fri", "every other Monday", "every Mon, Wed, Fri", "every Mon to Fri", "repeat on Fridays", "on Mondays and Thursdays"), frequency intervals (e.g. "every 3 days", "every other week", "repeat daily", "weekly", "every month", "yearly"), and duration-based recurrences like "for next 4 days" or "daily for 7 days" (which automatically start today at default 7:00 AM and repeat through the specified duration). You can even specify an end date by appending "until [date]" or "ending on [date]".
...
- **`tests/RegressionTests.swift`:** Automated test suite running 145 assertions on parser logic, day-of-week recurrences, relative date calculation, typo fixes, tricky words, URL extraction, and recurrence rules.
